...
describe('Izhikevich Presets and Solver Physics', () => {
  test('Standard presets exist with correct biological constants', () => {
  ...
  test('stepIzh sub-threshold current does not spike', () => {
  ...
  test('stepIzh depolarizing current triggers action potential', () => {
...
describe('Hodgkin-Huxley Q10 Thermodynamics scaling', () => {
  test('getQ10Factor scales exponentially with temperature relative to reference', () => {
...
describe('3-Phase MaxInterval Burst Detection Accuracy', () => {
  test('Groups tight spike train clusters and filters isolated spikes', () => {
    // Mock spike train:
  ...
  test('Merges bursts closer than minIbi', () => {
...
describe('SynapticNetwork Simulator Class Integration', () => {
  test('Initializes with 64 electrodes and healthy bioreactor conditions', () => {
  ...
  test('Ticking network depletes glucose and propagates simulation time', () => {
    const net = new SynapticNetwork();
  ...
  test('Manual stimulation triggers spike and updates timing metrics', () => {
    const net = new SynapticNetwork();
